...
- **execute's `command` is an action name with backend executor resolution**: on Windows, the backend (`isWindowsExecutable` in `backend/filesystem.ts`) probes whether the command resolves to a directly spawnable executable; if not (cmd builtins like dir/start/del, `.bat`/`.cmd`), it is dispatched through `cmd /d /s /c` with the command string assembled by the backend. The cmd dispatch path must keep `assertNoCmdMetaChars` (rejects `& | < > ^` in command and args with `INVALID_ARGUMENT`) — do not bypass or weaken this guard when changing the executor logic.
